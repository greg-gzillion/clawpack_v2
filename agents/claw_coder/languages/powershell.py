...
class PowershellLanguage(BaseLanguage):
    name = "powershell"
    extensions = ['.ps1', '.psm1']
    compilers = ['powershell']
    
    def __init__(self):
        self.web = WebClawClient()
    
    def generate(self, prompt: str, context: str = "") -> str:
        full_prompt = f"Generate powershell code for: {prompt}"
    ...
    def _call_ai(self, prompt: str) -> str:
        try:
            ...
            if response.status_code == 200:
                return response.json().get("response", f"// powershell generation failed")
        except:
            pass
        return f'// Generated powershell code\n// ⚡ Language: powershell\n'
