...
class SqlLanguage(BaseLanguage):
    name = "sql"
    extensions = ['.sql', '.psql']
    compilers = ['sqlite3', 'postgres']
    
    def __init__(self):
        self.web = WebClawClient()
    
    def generate(self, prompt: str, context: str = "") -> str:
        full_prompt = f"Generate sql code for: {prompt}\n\nGenerate idiomatic sql code:"
        return self._call_ai(full_prompt)
    
    def analyze(self, code: str) -> dict:
        prompt = f"Analyze this sql code:\n\n{code}"
        response = self._call_ai(prompt)
        return {"issues": [], "suggestions": [response[:500]]}
    
    def refactor(self, code: str, suggestion: str) -> str:
        prompt = f"Refactor this sql code: {suggestion}\n\nCode:\n{code}"
    ...
    def _call_ai(self, prompt: str) -> str:
        try:
            ...
            if response.status_code == 200:
                return response.json().get("response", f"// sql generation failed")
        except:
            pass
        return f'// Generated sql code\n// 🗄️ Language: sql\n\nfunction main() {{\n    console.log("Hello, World!");\n}}\n'
