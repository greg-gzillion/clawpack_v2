...
class GroovyLanguage(BaseLanguage):
    name = "groovy"
    ...
    def generate(self, prompt: str, context: str = "") -> str:
        full_prompt = f"Generate groovy code for: {prompt}\n\nGenerate idiomatic groovy code:"
        return self._call_ai(full_prompt)
    
    def analyze(self, code: str) -> dict:
        prompt = f"Analyze this groovy code:\n\n{code}"
        response = self._call_ai(prompt)
        return {"issues": [], "suggestions": [response[:500]]}
    
    def refactor(self, code: str, suggestion: str) -> str:
        prompt = f"Refactor this groovy code: {suggestion}\n\nCode:\n{code}"
    ...
    def _call_ai(self, prompt: str) -> str:
        try:
            ...
            if response.status_code == 200:
                return response.json().get("response", f"// groovy generation failed")
        except:
            pass
        return f'// Generated groovy code\n// 🍜 Language: groovy\n\nfunction main() {{\n    console.log("Hello, World!");\n}}\n'
