...
class CobolLanguage(BaseLanguage):
    name = "cobol"
    extensions = ['.cob', '.cbl']
    compilers = ['cobc']
    ...
    def generate(self, prompt: str, context: str = "") -> str:
        full_prompt = f"Generate cobol code for: {prompt}"
    ...
    def _call_ai(self, prompt: str) -> str:
        try:
            ...
            if response.status_code == 200:
                return response.json().get("response", f"// cobol generation failed")
        except:
            pass
        return f'// Generated cobol code\n// 🏦 Language: cobol\n'
