...
class CsharpLanguage(BaseLanguage):
    ...
    def generate(self, prompt: str, context: str = "") -> str:
        full_prompt = f"Generate csharp code for: {prompt}\n\nGenerate idiomatic csharp code:"
        return self._call_ai(full_prompt)
    
    def analyze(self, code: str) -> dict:
        prompt = f"Analyze this csharp code:\n\n{code}"
        response = self._call_ai(prompt)
        return {"issues": [], "suggestions": [response[:500]]}
    
    def refactor(self, code: str, suggestion: str) -> str:
        prompt = f"Refactor this csharp code: {suggestion}\n\nCode:\n{code}"
    ...
    def _call_ai(self, prompt: str) -> str:
        try:
            ...
            if response.status_code == 200:
                return response.json().get("response", f"// csharp generation failed")
        except:
            pass
        return f'// Generated csharp code\n// 🔷 Language: csharp\n\nfunction main() {{\n    console.log("Hello, World!");\n}}\n'
