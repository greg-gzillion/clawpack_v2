...
class PhpLanguage(BaseLanguage):
    ...
    def generate(self, prompt: str, context: str = "") -> str:
        full_prompt = f"Generate php code for: {prompt}\n\nGenerate idiomatic php code:"
        return self._call_ai(full_prompt)
    
    def analyze(self, code: str) -> dict:
        prompt = f"Analyze this php code:\n\n{code}"
        response = self._call_ai(prompt)
        return {"issues": [], "suggestions": [response[:500]]}
    
    def refactor(self, code: str, suggestion: str) -> str:
        prompt = f"Refactor this php code: {suggestion}\n\nCode:\n{code}"
    ...
    def _call_ai(self, prompt: str) -> str:
        try:
            ...
            if response.status_code == 200:
                return response.json().get("response", f"// php generation failed")
        except:
            pass
        return f'// Generated php code\n// 🐘 Language: php\n\nfunction main() {{\n    console.log("Hello, World!");\n}}\n'
