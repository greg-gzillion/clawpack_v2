...
class BatchLanguage(BaseLanguage):
    name = "batch"
    extensions = ['.bat', '.cmd']
    compilers = ['cmd']
    ...
    def generate(self, prompt: str, context: str = "") -> str:
        full_prompt = f"Generate batch code for: {prompt}"
    ...
    def _call_ai(self, prompt: str) -> str:
        try:
            ...
            if response.status_code == 200:
                return response.json().get("response", f"// batch generation failed")
        except:
            pass
        return f'// Generated batch code\n// 💻 Language: batch\n'
