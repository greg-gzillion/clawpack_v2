...
class MatlabLanguage(BaseLanguage):
    name = "matlab"
    extensions = ['.m', '.mat']
    compilers = ['matlab']
    ...
    def generate(self, prompt: str, context: str = "") -> str:
        full_prompt = f"Generate matlab code for: {prompt}"
    ...
    def _call_ai(self, prompt: str) -> str:
        try:
            ...
            if response.status_code == 200:
                return response.json().get("response", f"// matlab generation failed")
        except:
            pass
        return f'// Generated matlab code\n// 📐 Language: matlab\n'
