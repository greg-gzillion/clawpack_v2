...
class TXClawA2AHandler(BaseAgent):
    ...
    def _gather_context(self, query=""):
        """Gather context from DataClaw (local docs) and WebClaw (web search)."""
        parts = []
    ...
    def handle(self, task: str) -> dict:
        ...
        try:
            if cmd == "/tx" and args:
                ...
                result = self._call("Get TX.org gas fees structure. TX.org uses Cosmos SDK fee model. Explain how gas works on TX.org.")
            elif cmd == "/ecosystem":
                ...
                result = self._call(f"Generate a complete CosmWasm smart contract named {args!r} for TX.org blockchain in Rust.")
            elif cmd == "/deploy" and args:
                result = self._call(f"Provide step-by-step instructions to deploy CosmWasm contract {args!r} to TX.org testnet.")
            elif cmd == "/test" and args:
                result = self._call(f"Generate comprehensive unit tests for CosmWasm contract {args!r} on TX.org blockchain in Rust.")
            elif cmd == "/networks":
