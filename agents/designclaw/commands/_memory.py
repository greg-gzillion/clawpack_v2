"""designclaw shared memory helper — constitutional cross-command learning."""
...
def _log(event, detail=""):
    try:
        ...
        log_event(agent="designclaw", event=event, detail=str(detail)[:500])
    except Exception:
        pass
...
def remember(command: str, query: str, result_summary: str,
    ...
    try:
        ...
        check = sanitize_memory_write("designclaw", result_summary[:100], source_type, confidence)
        if not check.get("allowed"):
            return False
        from shared.memory.unified_memory import UnifiedMemory
        mem = UnifiedMemory()
        fact = {
            "agent": "designclaw", "command": command, "query": query,
            "fact": result_summary[:500], "source_type": source_type,
            "confidence": confidence, "urls": urls or [],
