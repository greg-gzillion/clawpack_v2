"""hospital command - Hospital lookup via jurisdiction files + Chronicle + LLM"""
import re
...
def run(args):
    ...
    try:
            ...
            for county_dir in sorted(state_dir.iterdir()):
                ...
                for city_dir in sorted(county_dir.iterdir()):
                    ...
                    for md_file in sorted(city_dir.iterdir()):
                        ...
                        if "hospital" in content.lower() or "medical center" in content.lower():
        ...
        if not found:
            out.append(f"  No hospital data found for {args}.")
            out.append(f"  Try: /jurisdiction {args} for full civic profile")
            return "\n".join(out)

        chronicle_ctx = chronicle_context(f"{args} hospital medical center", limit=5)

        combined = "\n\n".join(block[:1500] for block in all_content[:3])
        prompt = f"""Extract hospital information for: {args}
...
List each hospital with: name, address, phone, GPS coordinates, website.
Use ONLY the data provided. Under 200 words."""
        ...
        log("hospital_run_error", str(e)[:300])
        out.append(f"\n[ERROR] {str(e)[:300]}")
        return "\n".join(out)