...
class DraftClawAgent(BaseAgent):
    ...
    def _gather_context(self, query=""):
        parts = []
        web = self.call_agent("webclaw", f"search ns:draftclaw {query}", timeout=15)
    ...
    def _fileclaw_export(self, fmt, content):
        ...
        try:
            ...
            self.log(f"FileClaw export error: {e}")
    ...
    def _geo_text(self, jur_data):
        ...
        nl = chr(10)
        lines = [
            "## Jurisdiction-Specific Design Assumptions",
    ...
    def handle(self, task):
        ...
        try:
            ...
            if cmd in ("/stats",):
                return {"status":"success","result":f"DraftClaw v5 | Blueprints+CAD+Circuits | Interactions: {self.state.get('interactions',0)}"}
            ...
            if cmd in ("/permit","/compliance") and query:
                ...
                if jur_data.get("name") == "UNRESOLVED" or jur_data.get("error"):
                    return {"status":"error","result":f"**ERROR: Jurisdiction not found.**\n\nNo building code reference matched your query.\nTry: /lookup CITY STATE to find a jurisdiction.\n\nQuery: {query}\n"}
                code_refs = {
                ...
                if refs:
                    prompt = f"Reference codes:{nl}{refs[:3000]}{nl}{nl}{prompt}"
                result = self._filter_fake_engineering(self.ask_llm(prompt))
                if jur_data.get('contact', {}).get('url'):
                    try:
                        webbrowser.open(jur_data['contact']['url'])
