"""crustyclaw shared memory helper — constitutional cross-command learning.

Import in any crustyclaw command:
    from agents.crustyclaw.commands._memory import recall, remember, show_prior
"""
...
def _log(event, detail=""):
    try:
        ...
        log_event(agent="crustyclaw", event=event, detail=str(detail)[:500])
    except Exception:
        pass


def recall(query: str, limit: int = 5) -> list:
    """Search unified memory for prior Rust audits/patterns."""
    try:
...
def remember(command: str, query: str, result_summary: str,
    ...
    """Write an audit finding or Rust pattern to unified memory."""
    try:
        from shared.memory_guard import sanitize_memory_write
        check = sanitize_memory_write("crustyclaw", result_summary[:100], source_type, confidence)
