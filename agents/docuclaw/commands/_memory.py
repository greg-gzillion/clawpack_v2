"""docuclaw shared memory helper — constitutional cross-command learning."""
from datetime import datetime, timezone
from typing import Optional
def _log(event, detail=""):
    try:
        from agents.webclaw.core.chronicle_ledger import log_event
        log_event(agent="docuclaw", event=event, detail=str(detail)[:500])
...
def remember(command: str, query: str, result_summary: str,
    ...
    try:
        ...
        check = sanitize_memory_write("docuclaw", result_summary[:100], source_type, confidence)
        if not check.get("allowed"): return False
        from shared.memory.unified_memory import UnifiedMemory
        mem = UnifiedMemory()
        fact = {"agent": "docuclaw", "command": command, "query": query,
