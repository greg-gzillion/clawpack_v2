...
def derivative(args=None):
    ...
    if args:
        try:
            # Strip 'derivative' prefix if present
            expr_str = args.replace('derivative ', '').strip()
            ...
            return f"Derivative of {expr_str} wrt {var}: {result}"
...
def integral(args=None):
    ...
    if args:
        try:
            expr_str = args.replace('integral ', '').strip()
            expr = sp.sympify(expr_str)
            result = sp.integrate(expr)
            return f"Integral of {expr_str}: {result} + C"
        except Exception as e:
            return f"Error: {str(e)}"
