"""fileclaw shared memory helper — constitutional cross-command learning."""
...
def _log(event, detail=""):
    try:
        ...
        log_event(agent="fileclaw", event=event, detail=str(detail)[:500])
...
def remember(command: str, query: str, result_summary: str,
    ...
    try:
        ...
        check = sanitize_memory_write("fileclaw", result_summary[:100], source_type, confidence)
        if not check.get("allowed"): return False
        from shared.memory.unified_memory import UnifiedMemory
        mem = UnifiedMemory()
        fact = {"agent": "fileclaw", "command": command, "query": query,
